...
<body onload="mueveReloj()">
	<div class="container-fluid" style="background-color: #FFC300;">
		...
		<div class="col d-flex align-items-center"><h1>Mostrar Asesorias</h1></div>
		</div>
	</div>
      ...
      <li class="nav-item dropdown active">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          <a class="dropdown-item" href="../Asesorias.php">Registrar Asesoria</a>
            <a class="dropdown-item disabled" href="MostrarAseso.php">Mostrar Asesoriass</a>
        </div>
      </li>
      <!--Final de el Menu despegable-->
...
<div class="container">
	<div class="row">
		...
		<table border="1px" class="table table-warning table-hover table-bordered table-responsive">
			...
			<td><h6>Genero</h6></td>
			<td><h6>Grupo</h6></td>
			<td><h6>Calificacion</h6></td>
			<td><h6>Descripcion del la Asesoria</h6></td>
			<td><h6>Fecha del Registro</h6></td>
		</thead>
			<?php
				if($row = mysqli_fetch_array($result1)){
					do {
						echo "<tr><td>".$row["Nombre_Tutor"]."</td><td>".$row["Nombre_Alumno"]."</td><td>".$row["Motivo_Asesoria"]."</td><td>".$row["Genero"]."</td><td>".$row["Grupo"]."</td><td>".$row["Calificacion"]."</td><td>".$row["Descripcion_Motivo"]."</td><td>".$row["Fecha"]."</td></tr>"; 
					} while ($row = mysqli_fetch_array($result1));
				}else
				echo "<tr><td>No hay alguna Asesoria :'v</tr></td>";
			?>
		</table>
		</div>
	</div>
